    ...
    public String getHint(String secret, String guess) {
        ...
        for (int i = 0; i < secret.length(); i++) {
            if (secret.charAt(i) == guess.charAt(i)) {
                ...
                secretMap.put(c, secretMap.containsKey(c) ? secretMap.get(c) + 1 : 1);
                c = guess.charAt(i);
                guessMap.put(c, guessMap.containsKey(c) ? guessMap.get(c) + 1 : 1);
            }
        }

        for (Map.Entry<Character, Integer> secretEntry : secretMap.entrySet()) {
            char key = secretEntry.getKey();
            if (guessMap.containsKey(key)) {
                cows += Math.min(secretEntry.getValue(), guessMap.get(key));
            }
        }
